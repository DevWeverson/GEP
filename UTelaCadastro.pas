...
procedure TFcadastroproduto.AtualizarProduto(ID: Integer);
   ...
   begin
       ...
       DM.FDQuery.SQL.Add('PVenda = :precovenda WHERE CODPRO = :id');
       ...
       DM.FDQuery.ParamByName('precocusto').AsFloat := StrToFloat(Campoprecocusto.Text);
       DM.FDQuery.ParamByName('precovenda').AsFloat := StrToFloat(Campoprecovenda.Text);
       DM.FDQuery.ParamByName('unidade').AsString := CampoUnidade.Text;
       DM.FDQuery.ParamByName('id').AsInteger := StrToInt(campocodigo.Text);
...
procedure TFcadastroproduto.btSalvarClick(Sender: TObject);
  ...
  begin
   ...
   ID := Campocodigo.Text;
    ...
    if Action = 'incluir' then
    begin
      InserirNovoProduto;
    end

    else if Action = 'atualizar' then
